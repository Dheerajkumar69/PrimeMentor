...
export const convertAuTimeToIndiaDisplay = (dateString, timeSlot) => {
    ...
    /**
     * Convert a Melbourne time to IST using Intl.DateTimeFormat.
     * Probes UTC+10 (AEST) and UTC+11 (AEDT) to find the correct offset.
     */
    const convertTimeToIST = (timeStr) => {
        const time24h = get24HourTime(timeStr);
        ...
        for (const offset of [11, 10]) {
            const utcMs = Date.UTC(year, month - 1, day, h - offset, m, 0);
            const probe = new Date(utcMs);
            const parts = melbFormatter.formatToParts(probe);
            const getPart = (type) => parseInt(parts.find(p => p.type === type)?.value || '0', 10);

            if (getPart('hour') === h && getPart('minute') === m &&
                getPart('day') === day && getPart('month') === month) {
                return istFormatter.format(probe);
            }
        }

        // Fallback: approximate (Melbourne is ~5-5.5h ahead of IST)
        const approxHour = ((h - 5) % 24 + 24) % 24;
        const approxMin = (m + 30) % 60;
        const period = approxHour >= 12 ? 'PM' : 'AM';
        const displayH = approxHour % 12 || 12;
        return `${displayH}:${String(approxMin).padStart(2, '0')} ${period}`;
    };

    const startIST = convertTimeToIST(startTimeStr);
    const endIST = convertTimeToIST(endTimeStr);

    if (!startIST || !endIST) return 'N/A';
